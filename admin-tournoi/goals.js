...
// window.dislay_goals = fetchGoals(matchId);
...
function displayGoalsInTimeline(goals, matchId) {
  ...
  const existingGoalEvents = document.querySelectorAll(
  ...
  goals.forEach((goal) => {
    ...
    switch (goal.goal_type) {
      case "penalty":
        goalTypeText = "(Penalty)";
        break;
      case "own-goal":
        goalTypeText = "(Own Goal)";
        break;
      case "free-kick":
        goalTypeText = "(Free Kick)";
        break;
      case "header":
        goalTypeText = "(Header)";
        break;
    ...
    eventItem.innerHTML = `
            <div class="event-time">
                <input type="number" class="event-dte" value="${
                  goal.goal_time
                }" readonly>
            </div>
            <div class="event-team">
                <select class="event-team-select" disabled>
                    <option value="${teamValue}" selected>${
      isHomeTeam
        ? document.getElementById("detail-home-team").textContent
        : document.getElementById("detail-away-team").textContent
    }</option>
                </select>
            </div>
            ...
            <div class="event-player">
                <select class="player-select" disabled>
                    <option value="${goal.player_id}" selected>${
      goal.scorer_name || "Unknown Player"
    }</option>
                </select>
            </div>
            <div class="event-details goal-details">
                <select class="goal-type" disabled>
                    <option value="${goal.goal_type}" selected>${
      goalTypeText || "Normal"
    }</option>
                </select>
            </div>
            <div class="event-assist" ${
              !goal.assist_player_id ? 'style="display: none;"' : ""
            }>
                <select class="assist-player-select" disabled>
                    <option value="${goal.assist_player_id}" selected>${
      goal.assist_name || "No assist"
    }</option>
                </select>
            </div>
            <button type="button" class="remove-goal" data-goal-id="${
              goal.goal_id
            }">×</button>
        `;
...
window.displayGoalsInTimeline = displayGoalsInTimeline;
...
// a fucntiion to get the score of a match based on the match id , and return the score in as score.home/away format
...
// const goalDatafor21= get_score(21)
// console.log('thissssssssssssssss the score for 25')
// console.log(goalDatafor21.home)
...
function updateScoreDisplay(matchId, goals) {
  ...
  const homeTeamId = matchCard
    ? parseInt(matchCard.getAttribute("data-home-team-id"))
    : null;
  const awayTeamId = matchCard
    ? parseInt(matchCard.getAttribute("data-away-team-id"))
    : null;
...
function saveNewGoals(matchId) {
  ...
  const newGoalEvents = Array.from(
    document.querySelectorAll("#match-events .event-item")
  ).filter(
    (item) =>
  ...
  const savePromises = newGoalEvents.map((eventItem) => {
    ...
    const assistPlayerId =
      eventItem.querySelector(".assist-player-select").value || null;
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (saveDetailsButton) {
    ...
    saveDetailsButton.addEventListener("click", function (e) {
      ...
      saveNewGoals(matchId).then((success) => {
        if (success) {
